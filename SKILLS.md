...
3. Expected output: Two containers running (symbi-coordinator, symbi-a2ui); LanceDB is embedded in symbi (no separate container). a2ui proxies API calls to symbi:8081 via custom Caddyfile. Use `make desktop-up-replicated` to add Litestream GCS replication.
